...
$mapData = [
    ...
    "Latitude" => "latitude",
    "Longitude" => "longitude",
    "Wheat Procurement" => "wheat_procurement",
    ...
    //if (isset($_POST["submit"])){
        ...
        if ($_FILES["file"]["size"] > 0) {
            ...
            while (($column = fgetcsv($file, 10000, ",")) !== FALSE) {
                if($i>0){
                    if($district<0 or $name<0 or $id<0 or $wheat_procurement<0 or $latitude<0 or $longitude<0 or $active<0){
                    ...
                    if (!isStringNumber($column[$wheat_procurement])) {
                        echo "Error : Check Wheat Procurement Value<br>";
                        $redirect = 0;
                    }
                    ...
                    filterData($column[$wheat_procurement]);
                    ...
                    $PC->setId($column[$id]);
                    $PC->setWheatProcurement($column[$wheat_procurement]);
                    ...
                    for($j=0;$j<count($column);$j++){
                        switch($column[$j]){
                            ...
                            case $reverseMapData["wheat_procurement"]:
                                $wheat_procurement = $j;
                                break;
