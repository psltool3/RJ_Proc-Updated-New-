...
$mapData = [
    ...
    "Latitude" => "latitude",
    "Longitude" => "longitude",
    "Available Capacity(Qtl)" => "requirement",
    "Storage(Qtl)" => "storage_capacity",
...
$allowedWarehouseTypes = [];
$query_pref = "SELECT name FROM preferences";
$result_pref = mysqli_query($con, $query_pref);
if ($result_pref && mysqli_num_rows($result_pref) > 0) {
    while ($row_pref = mysqli_fetch_assoc($result_pref)) {
        $allowedWarehouseTypes[] = trim($row_pref["name"]);
    }
}
		...
		while (($column = fgetcsv($file, 10000, ",")) !== FALSE) {
			if($i>0){
				...
				if(!isStringNumber($column[$requirement])){
					echo "Error : Check Available Capacity(Qtl) Value: ".$column[$requirement];
					echo "</br>";
					$redirect = 0;
				}
                if(!isStringNumber($column[$storage_capacity])){
					echo "Error : Check Storage(Qtl) Value: ".$column[$storage_capacity];
					echo "</br>";
					$redirect = 0;
				}
				...
				$uploadedType = trim($column[$warehousetype]);
				$isValidType = in_array($uploadedType, $allowedWarehouseTypes, true);

				if(!$isValidType){
					echo "Error : Check Warehouse Type for ID: ".$column[$id].", Value: ".$column[$warehousetype].". Allowed values are: " . implode(', ', $allowedWarehouseTypes);
					echo "</br>";
					$redirect = 0;
				}
			}
