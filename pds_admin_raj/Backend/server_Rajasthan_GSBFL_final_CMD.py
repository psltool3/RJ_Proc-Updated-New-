...
def connect_to_database():
    ...
    user = 'root'
    password = ''
    database = 'rajasthan_procurement'
...
def extract_db():
    if request.method == 'POST':
        ...
        if connection.is_connected():
            ...
            if user:
                for row in user:
                    temp = {'State Name':'','WH_District': row[1], 'WH_Name': row[2], 'WH_ID': row[3], 'WH_Lat': row[4], 'WH_Long': row[5], 'Wheat_Procurement': row[6],}
        ...
        wh = wh.loc[:,["State Name","WH_District",'WH_Name',"WH_ID",'WH_Lat',"WH_Long","Wheat_Procurement"]]
        fps = fps.loc[:,["State Name","FPS_District",'FPS_Name',"FPS_ID","Type",'FPS_Lat',"FPS_Long","Storage","Demand"]]
...
def fci_data():
    try:
        ...
        total_demand = float(fci['Wheat_Procurement'].sum())
        
        milling_Capacity = float(fps['Storage'].sum())
        
        incoming_min = float(fps['Demand'].sum())
...
def graph_data():
    try:
        ...
        District_Procurement = {}
        for i in range(len(FCI["WH_District"])):
            District_Name = FCI["WH_District"][i]
            if District_Name not in District_Procurement:
                District_Procurement[District_Name] = float(FCI["Wheat_Procurement"][i])
            else:
                District_Procurement[District_Name] += float(FCI["Wheat_Procurement"][i])
        ...
        for i in range(len(FPS["FPS_District"])):
            ...
            if District_Name_FPS not in District_Capacity:
                District_Capacity[District_Name_FPS] = float(FPS["Storage"][i])
            else:
                District_Capacity[District_Name_FPS] += float(FPS["Storage"][i])
        ...
        for i in range(len(FPS["FPS_District"])):
            ...
            if District_Name_FPS not in District_Demand:
                District_Demand[District_Name_FPS] = float(FPS["Demand"][i])
            else:
                District_Demand[District_Name_FPS] += float(FPS["Demand"][i])

        # Infeasibility check: Procurement vs Warehouse Capacity
        District_Name = [
            i for i in District_Procurement
            if District_Procurement[i] > District_Capacity.get(i, 0)
        ]
        ...
        combined_data = {
            'District_Procurement': District_Procurement, 
...
def save_to_database(month, year, day):
    ...
    if connection.is_connected():
        ...
        connection.commit()
        create_pc_query = ("CREATE TABLE " + pc_table + " (uniqueid VARCHAR(100) NOT NULL, district VARCHAR(100) NOT NULL, name VARCHAR(100) NOT NULL, id VARCHAR(100) NOT NULL, latitude VARCHAR(100) NOT NULL, longitude VARCHAR(100) NOT NULL, wheat_procurement VARCHAR(100) NOT NULL, active VARCHAR(10) NOT NULL DEFAULT '1')")
...
def processFile():
    ...
    if scenario_type is not None and scenario_type.lower() == "intra":
        ...
        # Create dictionary in required format
        N = {}
...
# Close connection
        ...
        try:
            ...
            data = {}
            data['status'] = 0
            data['message'] = str(e)   # exact error message
        ...
        USN = pd.ExcelFile('Backend//Data_1.xlsx')
        FCI = pd.read_excel(USN, sheet_name='A.1 Warehouse', index_col=None)
        print(FCI['Wheat_Procurement'].sum())
        FPS = pd.read_excel(USN, sheet_name='A.2 FPS', index_col=None)
        FCI.rename(columns={'Wheat_Procurement': 'Storage_Capacity'}, inplace=True)
        print(FCI['Storage_Capacity'].sum())


        for n in N:
            print(n)
            FPS_N = FPS[FPS['Type'] == N[n]].reset_index(drop=True)
            FPS_N.rename(columns={'Demand': 'Allocation_Wheat'}, inplace=True)
            node1 = FCI
            node2 = FPS_N
            dist = [[0 for a in range(len(node2["FPS_ID"]))] for b in range(len(node1["WH_ID"]))]
            phi_1 = []
        ...
        #     print(allCombination1)
            ...
            TotalCapacity=FPS_N['Storage'] .sum()
        ...
        if dfs:
            ...
            df.to_csv("Backend//Inter_District1.csv", index=False,header=False)
        else:
        ...
        df4 = pd.read_excel('Backend//Tagging_Sheet_Pre.xlsx')
        ...
        df51.insert(2, 'From_State', 'Rajasthan')
        df51.insert(7, 'To', 'Mill')
        df51.insert(8, 'To_State', 'Rajasthan')
...
# Close connection
        ...
        if 'Wheat_Procurement' in FCI.columns:
            FCI.rename(columns={'Wheat_Procurement': 'Storage_Capacity'}, inplace=True)
