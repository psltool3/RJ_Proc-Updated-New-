...
<ul class="breadcrumb">
	<li><a href="#">Home</a></li>
	<li class="active">Rajasthan PDS Movement Optimisation</li>
</ul>
<!-- END BREADCRUMB -->
<div>
		...
		<div class="col-md-12">
			...
			<div class="panel panel-default">
				<div class="panel-heading" style="text-align: center;">
					...
					<h1 style="font-weight: bold; color: #FF6666;">Kindly Optimised the Leg1-PC to Warehouse</h1>
							...
							<div class="row">
								...
								<div class="col-md-4 mb-4">
									<div class="card h-100"
										...
										<div style="font-size:20px" id="total_warehouse_leg2"></div>
								...
								<div class="col-md-4 mb-4">
									<div class="card h-100"
										...
										<div style="font-size:20px" id="capacity_leg2"></div>
										<div style="font-size:14px">Total Storage (Qtl)</div>
									</div>
								</div>
								...
								<div class="col-md-4 mb-4">
									<div class="card h-100"
										...
										<div style="font-size:20px" id="total_demand_leg2"></div>
										<div style="font-size:14px">Total Available Capacity (Qtl)</div>
									</div>
								</div>							
							</div>
						</div>
...
<script>
				...
				try{
					...
					fetch(pythonUrl + 'extract_db', {
						...
						.then(data => {
									...
									document.getElementById("total_warehouse_leg2").innerHTML = formatNumberWithCommasWithoutDecimal(data["FPS_No"]);
									document.getElementById("capacity_leg2").innerHTML = formatNumberWithCommasWithoutDecimal(data["Milling_Capacity"]);
									document.getElementById("total_demand_leg2").innerHTML = formatNumberWithCommasWithoutDecimal(data["Incoming_Min"]);
	...
	function handleOptimizationResult(data) {
		...
		var headers = ["Scenario", "PC_Used", "WH_Used", "Total_Allocation", "Total_QKM", "Average Distance"];
	...
	if (checkbox.checked) {
		...
		fetch(pythonUrl + 'getGraphData', {
			...
			.then(data => {
				...
				var totalProcurement = Object.values(data.District_Procurement).reduce((acc, val) => acc + val, 0);
				var totalCapacity = Object.values(data.District_Capacity).reduce((acc, val) => acc + val, 0);
				var totalDemand = Object.values(data.District_Demand).reduce((acc, val) => acc + val, 0);
				var today_date = document.getElementById("today_date").value;

				// Format values with commas
				var formattedTotalProcurement = totalProcurement.toLocaleString();
				var formattedTotalCapacity = totalCapacity.toLocaleString();
				var formattedTotalDemand = totalDemand.toLocaleString();

				document.getElementById("totalFciDemand").innerHTML = "<span style='color: white; font-size: 14px;'>" + "Total Available Capacity: " + formattedTotalDemand + " (Qtl)</span>";
				document.getElementById("totalFciSupply").innerHTML = "<span style='color: white; font-size: 14px;'>" + "Total Wheat Procurement: " + formattedTotalProcurement + " (Qtl)</span>";
				document.getElementById("totalFciDemand1").innerHTML = "<span style='color: white; font-size: 14px;'>" + "Total Storage: " + formattedTotalCapacity + " (Qtl)</span>";
				...
				if (totalCapacity > -2 && totalDemand > -2) {
					if (totalProcurement <= totalCapacity) {
						document.getElementById("result").innerHTML =
					...
					var districtNamesProcurement = Object.keys(data.District_Procurement);
					var districtNamesCapacity = Object.keys(data.District_Capacity);
					var districtNamesDemand = Object.keys(data.District_Demand);
					
					const unionSet = new Set([...districtNamesProcurement, ...districtNamesCapacity, ...districtNamesDemand]);
					const unionArray = Array.from(unionSet);

					var procurements = unionArray.map(district => data.District_Procurement[district] || 0);
					...
					var newData = {
						...
						datasets: [
							{
								label: 'Total Wheat Procurement(Qtl)',
								backgroundColor: '#EFB838',
								data: procurements
							},
							{
								label: 'Total Storage (Qtl)',
								backgroundColor: '#3F51B5',
								data: capacities
							},
							{
								label: 'Total Available Capacity (Qtl)',
								backgroundColor: '#25E6A5',
								data: demands
							}
						]
