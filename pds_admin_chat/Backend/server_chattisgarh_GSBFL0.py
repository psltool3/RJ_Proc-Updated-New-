...
# ----------------------------------------------------------------------------------------------------------
    ...
    # ================= MILL SUPPLY =================
       ...
       # Calling CBC_CMB Solver
        ...
        FCI = pd.read_excel(USN, sheet_name='A.2 FCI', index_col=None)
        Mil = pd.read_excel(USN, sheet_name='A.2 Mill', index_col=None)

        # Convert WH_ID to string
        FCI['WH_ID'] = FCI['WH_ID'].astype(str)
        Mil['WH_ID'] = Mill['WH_ID'].astype(str)

        # Drop extra column from Mill
        Mil = Mil.drop(columns=['To_District'], errors='ignore')

        # Concatenate both DataFrames
        combined_df1 = pd.concat([FCI, Mil], ignore_index=True)
        ...
        df44 = pd.merge(df31, combined_df1, on='WH_ID', how='inner')
        #df4 = pd.merge(df31, FCI, on='WH_ID', how='inner')
        df44 = df44[[
            'WH_ID',
            'WH_Name',
        ...
        # print(df4)
        df4 = pd.merge(df44, WH, on='SW_ID', how='inner')
        df51 = df4[[
            'WH_ID',
            'WH_Name',
            'WH_District',
