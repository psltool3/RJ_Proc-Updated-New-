...
if($val_requirement > $val_storage_capacity){
	$errors[] = "Error : Available Capacity(Qtl) should not be greater than Storage(Qtl)";
}
...
$uploadedType = trim($_POST["warehousetype"] ?? '');
$isValidType = in_array($uploadedType, $allowedWarehouseTypes, true);

if (!$isValidType) {
    $errors[] = "Error : Invalid Warehouse Type. Allowed values are: " . implode(', ', $allowedWarehouseTypes);
}
